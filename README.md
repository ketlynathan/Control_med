...
O projeto fica na raiz do repositório porque o Vercel está conectado à raiz da branch `main`. Manter a aplicação dentro de uma subpasta faria o deploy retornar `404`.
...
| `GROQ_API_KEY` | sua chave da API da Groq (opcional, só para o botão "Colar nota") | [console.groq.com/keys](https://console.groq.com/keys) — veja a seção 4 abaixo |
...
### 4. (Opcional, mas recomendado) Chave da API da Groq — para o botão "Colar nota"
...
- Acesse [console.groq.com](https://console.groq.com) e abra **API Keys**.
- Crie uma chave para o projeto e adicione-a como variável de ambiente `GROQ_API_KEY` na Vercel.
...
O modelo usado é `llama-3.3-70b-versatile`, chamado pela API compatível com OpenAI da Groq e configurado para retornar JSON estruturado.
...
- **"Colar nota" exige login** (mesma proteção do resto do app) para que ninguém de fora consiga gastar créditos da sua chave da Groq. Ainda assim, qualquer colaborador que tenha login no seu Fluxo pode usar o botão — se isso for um problema, dá para restringir por papel do colaborador no futuro.
